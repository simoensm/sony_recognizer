    ...
    <form onSubmit={save} className="panel mt-4 flex flex-col gap-4 border border-white/10 p-6">
      <label className="flex flex-col gap-1.5 text-sm">
        ...
        <input
          ...
          required
          minLength={2}
          className="border border-white/20 bg-transparent px-4 py-2 focus:border-white focus:outline-none"
        />
      </label>
      <label className="flex flex-col gap-1.5 text-sm">
        ...
        <input
          value={email}
          disabled
          className="border border-white/10 bg-white/5 px-4 py-2 text-white/40"
        />
      ...
      {msg && (
        <p className={`text-sm ${msg.ok ? "text-white/70" : "text-red-400"}`}>{msg.text}</p>
      )}
      <button
